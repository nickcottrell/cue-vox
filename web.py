...
def _is_fingerprint_query(text):
    """True for 'what's your voice fingerprint / voice id / voice hash / voice stamp'.
    Requires 'fingerprint' outright, or 'voice' paired with an id-ish word, so ordinary
    talk about voices does not trip it."""
    low = (text or "").lower()
    if "fingerprint" in low:
        return True
    return bool("voice" in low and re.search(r"\b(id|hash|stamp|code|signature|checksum)\b", low))


def _is_ladder_command(text):
    """True for 'run/walk the ladder' -- the deterministic voice ladder walk. Requires
    'ladder' so normal talk does not trigger it."""
    low = (text or "").lower()
    if "ladder" not in low:
        return False
    return any(v in low for v in ("run", "walk", "go", "do", "step", "through", "start", "play", "climb"))


def _answer_fingerprint():
    """Answer the LIVE voice fingerprint deterministically -- read from the running config,
    never generated by the model, so cue-vox always returns the CORRECT id for the voice it
    is actually speaking with. Logs it and pushes it to the browser console too."""
    fp = voice_fingerprint()
    spoken = "My voice fingerprint is %s." % ", ".join(re.findall("..", fp[1:]))
    card = "Voice fingerprint: %s  (%s)" % (fp, _ACTIVE_VOICE)
    print("[VOICE] fingerprint query -> %s (%s)" % (fp, _ACTIVE_VOICE), flush=True)
    try:
        socketio.emit("voice_fingerprint", {"fingerprint": fp, "voice": _ACTIVE_VOICE})
    except Exception:
        pass
...
def _run_benchmark_walk():
    ...
    try:
        emit('state_change', {'state': 'speaking'})
        fp = voice_fingerprint()                       # the "same voice?" stamp
        fp_say = ", ".join(re.findall("..", fp[1:]))   # "a1, b2, c3" reads clearly aloud
        say("STARTING BENCHMARK TEST  ·  voice %s" % fp,
            "Starting benchmark test. Voice fingerprint. %s." % fp_say)
...
def _run_ladder_walk():
    """The LADDER WALK: a deterministic voice tour, no model. For each envelope it speaks a
    fixed script at floor (intensity 0) then ceiling (intensity 1), so you hear each band's
    spread and the climb between them; then a bridge line whose tags move it within one
    breath (hush -> spike -> beat -> peak bit). Expressive (Chatterbox) -- the moody engine
    we tune. Pins the envelope + forces the fill per rung, so it is the same every run."""
    global _live_mode, _expressive_mode, _register_lock, _forced_fill
    saved = (_live_mode, _expressive_mode, _register_lock, _forced_fill)
    _expressive_mode = True
    SCRIPT = "Here is the voice at this setting. Same words, this rung."
    BRIDGE = ('A normal line. <prosody volume="soft">a hush.</prosody> '
              '<emphasis level="strong">then this lands.</emphasis> '
              '<break time="500ms"/> <perform>and the bit.</perform>')
    ...
    def narrate(card, spoken):                     # narration at the chill floor, calm + clear
        global _register_lock, _forced_fill
        keys = _register_keys()
        _register_lock = keys[0] if keys else "1"
        _forced_fill = 0.0
        say(card, spoken)

    emit("bench_lock", {"locked": True})
    try:
        emit('state_change', {'state': 'speaking'})
        fp = voice_fingerprint()
        narrate("LADDER WALK  ·  voice %s" % fp,
                "Walking the ladder. Voice fingerprint. %s." % ", ".join(re.findall("..", fp[1:])))
        envs = _active_voice().get("envelopes", {}) or {}
        for key in _register_keys():
            e = envs.get(key, {}) or {}
            label = e.get("label", key)
            rng = _rng((e.get("chatterbox_profile") or {}).get("exaggeration", [1.0, 1.0]))
            blip()
            narrate("== %s (envelope %s) · exag %.1f to %.1f ==" % (label, key, rng[0], rng[1]),
                    "%s. Exaggeration %.1f to %.1f." % (label, rng[0], rng[1]))
            for name, inten in (("floor", 0.0), ("mid", 0.5), ("ceiling", 1.0)):
                _register_lock = key
                _forced_fill = inten
                exag = _map_range(rng, inten)
                say("%s · %s (intensity %.1f → exag %.2f)" % (label, name, inten, exag), SCRIPT)
        blip()
        mid = _register_keys()[len(_register_keys()) // 2] if _register_keys() else "2"
        narrate("== bridge · tags move within one line (envelope %s) ==" % mid,
                "And the bridge. One line, moving.")
        _register_lock = mid
        _forced_fill = 0.3
        say("bridge: hush → spike → beat → peak bit", BRIDGE)
        narrate("LADDER WALK COMPLETE", "That is the ladder.")
        emit('state_change', {'state': 'idle'})
    finally:
        _live_mode, _expressive_mode, _register_lock, _forced_fill = saved
        emit("bench_lock", {"locked": False})
    return None
...
_SLOW_OPS = {
    ...
    "ladder": {
        "confirm": "The ladder walk plays fixed scripts through every envelope at floor and ceiling, plus a bridge line. Expressive. Takes a moment. Start it?",
        "run": _run_ladder_walk,
    },
}
...
_forced_fill = None        # bench-only: pin the content fill (0..1) for a deterministic rung; None = normal
...
def _save_voices():
    ...
    try:
        ...
        return False
# Model vocabulary (user-facing): Personality > Envelope > Register > Voice.
#   Envelope = an energy band (chill/mid/peak): speed/bright/gain + prosody ranges +
#              chatterbox performer. THIS is what the render engine rides.
#   Register = a named mode inside an envelope; holds one or more Voices + a policy.
#   Voice    = the Kokoro leaf (a blend row / SID) -- the single source of truth for slot.
# The render engine below still names its state-key "register" (an internal render-state
# name, kept to avoid rewiring the working node); that key now indexes the personality's
# ENVELOPES. Read _active_register as "the active envelope tier".
def _active_voice():
...
_VOICE_REGISTERS = {"registers": _active_voice().get("envelopes", {})}
_register_lock = None  # 1/2/3 to pin that envelope (no autotone drift); None = autotone
...
def _prosody_for_slot(slot):
    """The active voice's prosody envelope for a live slot (0..4), chosen by the envelope
    whose Voice blend row is nearest. Empty dict (identity) if none defined."""
    regs = (_VOICE_REGISTERS or {}).get("registers", {}) or {}
    best, best_d = None, 99
    for r in regs.values():
        d = abs(_env_slot(r) - int(slot))
        if d < best_d:
            best, best_d = r, d
...
def set_active_voice(voice_id):
    ...
    _VOICE_REGISTERS = {"registers": v.get("envelopes", {})}
    if _kokoro_available:
        kokoro_voice.set_voice(sid=int(v.get("sid", 8)),
                               blend=v.get("blend"),
                               use_blend=bool(v.get("blend_slots")))
    _fp = voice_fingerprint(voice_id)
    print("[VOICE] active=%s sid=%s blend=%s blend_slots=%s fingerprint=%s"
          % (voice_id, v.get("sid"), v.get("blend"), bool(v.get("blend_slots")), _fp), flush=True)
    try:                                   # push the new fingerprint to any connected browser console
        socketio.emit("voice_fingerprint", {"fingerprint": _fp, "voice": voice_id})
    except Exception:
        pass
    ...
    regs = v.get("envelopes", {}) or {}
    ar = v.get("active_envelope")
...
def _max_render_slot():
    ...
    regs = (_VOICE_REGISTERS or {}).get("registers", {}) or {}
    return max([_env_slot(r) for r in regs.values()] or [4])
...
def set_render_register(reg, reason="", apply_exag=True, gated=True):
    ...
    # ONE CEILING: the old "register floor" clamp is retired. It pushed the live register
    # UP to a deployed base (e.g. a voice deployed with peak selected floored everything at
    # peak -- that was "peak taking over"). The resting-ceiling policy is the single control
    # now; a deploy sets the voice, not a clamping floor.
...
def _resting_ceiling():
    """ONE-CEILING fallback: the ceiling to hold before the track manager has settled (mid,
    one below the top). Once the concurrent tracker is running it owns the track; this is
    just the seed. The expressive gate still pins the lowest register when expressive is OFF."""
    keys = _register_keys()
    if not keys:
        return None
    return keys[max(0, len(keys) - 2)]


# === Envelope Track manager: the SLOW loop ==================================
# Two loops, separated by timescale:
#   FAST (within a track): the content fill, floor->ceiling, per line -- clamped to the
#         active track's range. (resolve_instructions + _exaggeration.)
#   SLOW (which track): a moving average of token-warmth ENERGY that regulates WHICH
#         Envelope Track is active, with hysteresis so a track is EARNED (energy crosses an
#         up-threshold) and RELEASED (energy deflates past a lower down-threshold). Because
#         it is slower than the fast loop, the track stays put while expression varies inside
#         its clamp -- a continuous but clamped environment. This is the async register
#         manager that replaces the old per-turn energy pool.
_TRACK_INDEX = 0            # current Envelope Track (index into _register_keys())
_TRACK_ENERGY = 0.0         # slow EMA of token-warmth energy (0..1) -- the DIRECTION signal
_TRACK_TICK_SEC = 1.2       # concurrent sample interval
_TRACK_EMA = 0.2            # EMA weight (lower = slower); the slow loop's smoothing
_TRACK_HOT = 82.0           # token temperature at/above which a token counts as "hot"
_TRACK_UP = [0.20, 0.33]    # heat climb thresholds per gap (chill->mid, mid->peak); energy must REACH
_TRACK_DOWN = [0.15, 0.27]  # heat drop thresholds per gap; energy must DEFLATE below (down < up = hysteresis)
# Emission / dwell pressure: staying on a track EMITS tokens; as they build, pressure rises
# to move OFF it (so it never stays too long). A min-dwell rate limit stops it switching too
# often. Both are the "not too long, not too often" balance.
_TRACK_MAX_DWELL = 8        # live emissions on the current track that force a switch (~a couple min of talk)
_TRACK_MIN_DWELL_SEC = 18.0 # no switch may fire within this many seconds of the last one
_TRACK_EMIT_HALFLIFE = 0.04 # emission-token half-life in hours (~2.5 min): pressure self-releases via decay
_TRACK_EMIT_FLOOR = 55.0    # an emission still counts toward pressure while above this temperature
_track_last_switch = 0.0    # wall-clock of the last switch (min-dwell gate)
_track_started = False


def _token_energy():
    """Conversational energy = the fraction of the cue-mem pool currently HOT (temperature
    at/above _TRACK_HOT). Rises as the conversation heats or mints tokens, deflates as they
    thermally decay -- an activity signal, not the static pool average."""
    ...
    try:
        ...
        return 0.0
    temps = [t.get("current_temperature") for t in toks if isinstance(t.get("current_temperature"), (int, float))]
    if not temps:
        return 0.0
    return sum(1 for x in temps if x >= _TRACK_HOT) / float(len(temps))


_emit_seq = 0


def emit_track(track):
    """Emission: speaking on a track mints a short-lived 'voice-emission' token tagged with
    it. Each emission is a DISTINCT token (unique label) so they accumulate -- as they build
    in the pool, dwell pressure rises to move OFF the track; thermal decay bleeds them once
    you leave, so the pressure releases on its own. Per turn spoken."""
    global _emit_seq
    if not (track and CUE_MEM_AVAILABLE):
        return
    _emit_seq += 1
    try:
        cue_mem_create_token("voice_emission_%d" % _emit_seq, track, token_type="scalar_param",
                             visibility="private", half_life_hours=_TRACK_EMIT_HALFLIFE,
                             base_temp=90, tags=["voice-emission", str(track)])
    except Exception:
        pass


def _dwell_pressure(track):
    """Dwell pressure for a track = how many of its emissions are still live (above the
    floor). Builds while you stay (each turn emits), deflates as they decay after you leave."""
    if not (track and CUE_MEM_AVAILABLE):
        return 0
    try:
        toks = cue_mem_list_tokens() or []
    ...
    for t in toks:
        tags = t.get("tags") or []
        temp = t.get("current_temperature", 0)
        if "voice-emission" in tags and str(track) in tags and isinstance(temp, (int, float)) and temp >= _TRACK_EMIT_FLOOR:
            n += 1
    return n


def _write_register_token(track):
    """The explicit, declared state: a 'voice_register' token whose value IS the current
    track -- readable by anyone, not inferred. Written on every switch."""
    if not (track and CUE_MEM_AVAILABLE):
        return
    try:
        cue_mem_create_token("voice_register", track, token_type="text",
                             visibility="shared", half_life_hours=1.0, base_temp=95,
                             tags=["voice-register", str(track)])
    except Exception:
        pass


# === Capabilities: Envelope x {tasks/tools} ================================
# A capability binds an envelope to a set of tasks/tools the personality is tuned for. It
# lives in the personality bundle (portable). At runtime the ACTIVE capability is found by
# matching active-work TOKENS (tokens tagged with a tool/task name) against each capability's
# set -- so the register follows what the system is actually DOING, grounded in fact.
# Seams left open: the tool tags use real MCP manifest names (v2: validate/populate from the
# live manifest), and a capability may later REFERENCE a Jeff-hosted policy instead of inlining.
def _active_capability():
    """The best-matching capability for the work currently hot in the token pool, or None
    (ambient -- no specific work, let the heat/dwell regulator wander)."""
    if not CUE_MEM_AVAILABLE:
        return None
    caps = _active_voice().get("capabilities") or []
    if not caps:
        return None
    try:
        toks = cue_mem_list_tokens() or []
    ...
    for t in toks:
        temp = t.get("current_temperature", 0)
        if isinstance(temp, (int, float)) and temp >= _TRACK_HOT:
            for tag in (t.get("tags") or []):
                hot_tags.add(str(tag).lower())
    best, best_score = None, 0
    for cap in caps:
        names = [str(x).lower() for x in ((cap.get("tools") or []) + (cap.get("tasks") or []))]
        score = sum(1 for n in names if n in hot_tags)
        if score > best_score:
            best, best_score = cap, score
    return best


def _cap_num(cap, key, default):
    """A numeric knob from a capability's local policy, falling back to the global default.
    (v2: if the policy is a Jeff reference, resolve it from the network here.)"""
    try:
        return float(((cap or {}).get("policy") or {}).get(key, default))
    except (TypeError, ValueError):
        return default


def _track_manager():
    """The slow loop, run concurrently: sample token-warmth energy, smooth it into an EMA,
    and move the Envelope Track by hysteresis (climb on reaching an up-threshold, drop on
    deflating past a down-threshold). Updates the DECISION only (_TRACK_INDEX); the render
    applies it at reply boundaries so a track never switches mid-utterance."""
    global _TRACK_ENERGY, _TRACK_INDEX, _track_last_switch
    while True:
        socketio.sleep(_TRACK_TICK_SEC)
        try:
            _TRACK_ENERGY = _TRACK_ENERGY * (1.0 - _TRACK_EMA) + _token_energy() * _TRACK_EMA
            keys = _register_keys()
            if not keys:
                continue
            i = max(0, min(_TRACK_INDEX, len(keys) - 1))
            cap = _active_capability()                  # what work is hot right now?
            min_dwell = _cap_num(cap, "min_dwell_sec", _TRACK_MIN_DWELL_SEC)
            if (time.time() - _track_last_switch) < min_dwell:
                continue                                # min-dwell: never switch too often
            target = i
            reason = None
            if cap and cap.get("envelope") in keys:
                # A capability owns the register: the active work sets the target envelope.
                # Step toward it (rate-limited), then HOLD -- work justifies staying, so the
                # anti-monotony wander is suppressed while a capability is active.
                tgt = keys.index(cap["envelope"])
                if tgt != i:
                    target = i + (1 if tgt > i else -1)
                    reason = "cap:" + str(cap.get("name", "?"))
                else:
                    continue
            else:
                # Ambient (no specific work): heat direction + dwell-pressure wander.
                pressure = _dwell_pressure(keys[i])
                want_up = i < len(keys) - 1 and _TRACK_ENERGY >= _TRACK_UP[min(i, len(_TRACK_UP) - 1)]
                want_down = i > 0 and _TRACK_ENERGY < _TRACK_DOWN[min(i - 1, len(_TRACK_DOWN) - 1)]
                forced = pressure >= _cap_num(cap, "max_dwell", _TRACK_MAX_DWELL)
                if want_up:
                    target = i + 1
                    reason = "heat"
                elif want_down:
                    ...
                    mid = (len(keys) - 1) / 2.0
                    target = i + (1 if i < mid else -1)
                    reason = "dwell"
            if reason and target != i and 0 <= target < len(keys):
                _TRACK_INDEX = target
                _track_last_switch = time.time()
                _write_register_token(keys[target])     # declare the new state explicitly
                pressure = _dwell_pressure(keys[i])
                _vlog("track", "energy=%.2f pressure=%d -> track %d (%s) [%s]"
                      % (_TRACK_ENERGY, pressure, target, keys[target], reason))
                try:
                    socketio.emit("voice_track", {"track": keys[target], "index": target,
                                                  "energy": round(_TRACK_ENERGY, 3),
                                                  "pressure": pressure, "reason": reason})
                except Exception:
                    pass
        except Exception as ex:
            print("[TRACK] %s" % ex, flush=True)


def _current_track_key():
    """The track the slow loop currently holds, as a register key (applied by the render at
    reply boundaries). Falls back to the resting seed before the tracker has moved."""
    keys = _register_keys()
    if not keys:
        return None
    return keys[max(0, min(_TRACK_INDEX, len(keys) - 1))]


def _start_track_manager():
    global _track_started
    if _track_started:
        return
    _track_started = True
    try:
        socketio.start_background_task(_track_manager)
        print("[TRACK] envelope-track manager started (tick=%.1fs ema=%.2f hot>=%.0f up=%s down=%s)"
              % (_TRACK_TICK_SEC, _TRACK_EMA, _TRACK_HOT, _TRACK_UP, _TRACK_DOWN), flush=True)
    except Exception as ex:
        _track_started = False
        print("[TRACK] failed to start: %s" % ex, flush=True)
...
def _render_ssml_to_wav(text):
    ...
    if len(instr) == 1 and "say" in instr[0]:    # single span -> flat synth
        it = instr[0]
        if it.get("perform"):                    # a lone <perform> span still shifts to peak
...
_GAIN_REF = 1.6   # loudness value that renders at full amplitude; higher bands' ceilings hit this


def _scale_wav_amp(path, amp):
    """Scale a wav's amplitude in place (the expressive GAIN lever, applied post-synth to a
    Chatterbox clip). amp <= 1.0 attenuates, so it never clips. No-op at unity."""
    if amp is None or amp >= 0.999:
        return
    try:
        ...
        a = np.frombuffer(frames, dtype=np.int16).astype(np.float32) * float(amp)
        a = np.clip(a, -32768, 32767).astype(np.int16)
        with _w.open(path, "wb") as w:
            w.setparams(params)
            w.writeframes(a.tobytes())
    except Exception as exc:
        print("[TTS] gain scale skipped: %s" % exc, flush=True)
...
def _render_expressive_to_wav(text, exaggeration):
    ...
    # THE BRIDGE: the Kokoro tags + policy compose a PER-SPAN performance score, and the
    # moody engine renders each span with its OWN exag/cfg. The active envelope's exaggeration
    # range is the mood frame; each span's parsed intensity (emphasis / volume / rate / lift,
    # over the reply's baseline fill) is WHERE in that range it lands. So one marked-up line
    # becomes hush -> build -> spike -> hush, performed, instead of one flat mood. <perform>
    # spans jump to the peak envelope. Runs split on an intensity-bucket change (or perform),
    # so the voice stays smooth without a synth call per word.
    # THREE ranges, ONE intensity: exag, cfg (pace), and gain (loudness) all lerp together
    # from the per-span intensity into the active band's [floor, ceiling]. Balanced climb --
    # every lever participates. gain is a loudness value applied post-synth as an amplitude
    # (attenuated below full, so it never clips); the loudest band's ceiling is full scale.
    prof = _active_chatterbox_profile()
    fill = max(0.0, min(1.0, float(exaggeration) - 1.0))     # the reply's baseline intensity
    pk = _peak_envelope().get("chatterbox_profile") or {}

    def _lever(p, key, inten, dflt):
        v = p.get(key)
        return _map_range(v, inten) if v is not None else dflt   # _map_range coerces scalar-or-range

    def _gain_amp(loud):
        return max(0.3, min(1.0, float(loud) / _GAIN_REF))   # loudness -> amplitude, attenuate-only

    def _exag_for(i):
        return _lever(prof, "exaggeration", i, 1.0 + i)

    def _cfg_for(i):
        return _lever(prof, "cfg_weight", i, 0.4)

    def _amp_for(i):
        return _gain_amp(_lever(prof, "gain", i, _GAIN_REF))

    def _peak_hi(key, dflt):
        v = pk.get(key)
        return _rng(v)[1] if v is not None else dflt
    perf_exag, perf_cfg = _peak_hi("exaggeration", 2.0), _peak_hi("cfg_weight", 0.7)
    perf_amp = _gain_amp(_peak_hi("gain", _GAIN_REF))
    ...
    if not any(("beat_ms" in o or "pause_ms" in o or "cue" in o or o.get("perform")) for o in ops) \
       and len({_bucket(x) for x in say_intens}) <= 1:        # one mood, no structure -> single call
        ...
        return p
    parts, buf = [], []
    run = {"perform": False, "inten": fill}                  # the run currently buffering
    ...
    for o in ops:
        if "say" in o:
            pf = bool(o.get("perform"))
            inten = float(o.get("intensity", fill))
            if buf and (pf != run["perform"] or _bucket(inten) != _bucket(run["inten"])):
                _flush_say()                    # perform OR intensity-bucket change -> new run
            if not buf:
                run["perform"], run["inten"] = pf, inten
            else:
                run["inten"] = max(run["inten"], inten)   # a run performs at its peak span
            buf.append(o["say"])
        elif "beat_ms" in o:
            _flush_say()
            fd, bp = tempfile.mkstemp(suffix=".wav", prefix="cvx-beat-"); os.close(fd)
...
# Spliced-in clips for [laugh]/[chuckle]/[giggle] markers -- an in-voice, lightly
# synthetic cue (articulated ha/hee/huh bursts pitched to the voice), not a faked
# human sample. Regenerate or retune via tools/make_laughs.py.
_SFX_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "sfx")
_LAUGH_RE = re.compile(r"\[(laugh|chuckle|giggle)\]", re.IGNORECASE)
...
def speak_chunked(text):
    ...
    # ONE CEILING: content fills UP TO the ceiling, it does not climb it. _exaggeration is
    # the fill -- how hard THIS reply pushes (its own weight, 0..1) -- mapped into the
    # ceiling register's exaggeration range. A calm line rests near the floor; a heavy one
    # reaches the ceiling; neither drags the register higher. Peak is a reserved gear
    # (<perform> / deliberate lift), not somewhere a heavy line wanders.
    _exaggeration = 1.0 + (max(0.0, min(1.0, _forced_fill)) if _forced_fill is not None
                           else max(0.0, min(1.0, w)))   # bench pins the fill; else content-driven
    if _register_lock is not None:
        set_render_register(_register_lock, "lock")        # bench/track pins an exact ceiling
    else:
        _trk = _current_track_key()
        set_render_register(_trk, "track")                  # apply the slow loop's current track
        emit_track(_trk)                                    # emit: speaking here builds dwell pressure
    try:
...
def _release_hold(data=None):
    ...
    try:
        ...
        set_render_register(_current_track_key() or "1", "release")
...
_TOPIC_PROMPT = (
    "Name what this exchange is about in 2 to 4 words. "
    "Output only the topic phrase: lowercase, no quotes, no punctuation, no trailing period. "
    "It is a stream label, not a sentence and not a summary of the reply."
)


def _summarize_exchange_topic(user_text, assistant_text):
    """Short topic label for the cue-stream, via the local model. Returns
    a cleaned 2-4 word phrase or None on any failure (caller falls back)."""
    u = (user_text or "").strip()
    a = (assistant_text or "").strip()
    if not u and not a:
        return None
    prompt = "%s\n\nUser: %s\nAssistant: %s\n\nTopic:" % (_TOPIC_PROMPT, u[:600], a[:600])
    try:
        from ollama_client import generate
        out = (generate(prompt, max_tokens=16, timeout=20, temperature=0.2) or "").strip()
    except Exception:
        return None
    if not out:
        return None
    # First line only; drop an echoed "Topic:" prefix and any wrapping quotes.
    out = out.splitlines()[0].strip()
    if out.lower().startswith("topic:"):
        out = out.split(":", 1)[1].strip()
    out = out.strip('"').strip("'").strip()
    # Label discipline: cap to 5 words, drop trailing punctuation.
    out = " ".join(out.split()[:5]).rstrip(".!?,;:").strip()
    return out or None
...
def log_conversation(user_text, assistant_text, speech_metadata=None, input_length=None, confidence=None):
    ...
    # Emit exchange timing token to keep the stream alive. The label is a
    # short topic summary of the turn instead of a static "exchange". Topic
    # generation runs off-thread (local model) so it never delays the spoken
    # response; the token lands a beat later, which is fine for the stream.
    _ex_ts = int(timestamp.timestamp())
    _ex_iso = timestamp.isoformat()
    _ex_hhmm = timestamp.strftime("%H:%M")
    _ex_user = entry.get("user", "")
    _ex_asst = clean_text

    def _emit_exchange_token():
        topic = _summarize_exchange_topic(_ex_user, _ex_asst) or "exchange"
        try:
            socketio.emit("token_created", {
                "token_id": "timing_exchange_%d" % _ex_ts,
                "type": "timing",
                "label": topic,
                "value": _ex_hhmm,
                "created_at": _ex_iso,
    ...
    try:
        socketio.start_background_task(_emit_exchange_token)
    except Exception:
...
def _ssml_pitch_off(v):
    """SSML prosody pitch -> a direction (-2..+2), used only to read pitch-up as a rise
    contour WITHIN the register. It no longer offsets the register (that climb was the
    old overload). Named low/high, or a signed +Nst / -N% reads as down/up by one step."""
    v = (v or "").strip().lower()
...
def _tag_contrib(tag, el):
    ...
    if t == "prosody":
        ...
        if g("pitch") is not None:
            # Pitch inflects WITHIN the current register, it never climbs to the next one.
            # The register/envelope is governed (chosen by policy + the expressive gate),
            # not shoved up by a per-span pitch tag. A pitch-up reads as the question/rise
            # contour; pitch-down stays neutral. No reg_off contribution.
            if _ssml_pitch_off(g("pitch")) > 0:
                c["lift"] = True
        return c
    if t == "voice":
        return _ssml_voice(g("name"))
    if t == "perform":
        # Wrap a span to perform it in the PEAK envelope (the performative gear) and drop
        # back after: an explicit, opt-in register shift for emphasis and bits. The render
        # resolves the flag to peak's blend + prosody (Kokoro) or peak's exaggeration
        # (Chatterbox). Governed: nothing climbs on its own; you point it.
        return {"perform": True}
    return {}
...
def _merge_ctx(ctx, c):
    ...
    for k in ("reg", "speed", "gain", "lift", "perform"):
        if k in c:
            n[k] = c[k]
    return n
...
# The sanctioned vocabulary: W3C SSML core + cvx extensions (beat/rest/laugh/chuckle/
# giggle/perform). Anything else is flagged unknown by the parse view (and stripped).
_KNOWN_TAGS = {"speak", "break", "pause", "beat", "rest", "emphasis", "prosody", "p", "s",
               "say-as", "sub", "voice", "laugh", "chuckle", "giggle", "perform"}


def _peak_envelope():
    """The top (peak) envelope of the active personality: the performative gear that
    <perform> reaches for. Highest envelope key; empty dict if none."""
    envs = (_VOICE_REGISTERS or {}).get("registers", {}) or {}
    keys = _register_keys()
    return envs.get(keys[-1], {}) if keys else {}


def _span_intensity(dv, base):
    """A span's expressive intensity (0..1) for the bridge: the reply's baseline (fill)
    pushed by the span's OWN tags -- strong emphasis / loud / fast / a question rise push up,
    reduced or soft emphasis pulls down. This is what the moody engine lerps into the active
    envelope's exaggeration range, per span, so the tags shape the neural performance."""
    inten = max(0.0, min(1.0, base))
    f = dv.get("force", 1.0)
    if f > 1.0:
        inten = max(inten, _emph_intensity(f))
    elif f < 1.0:
        inten *= f                                       # reduced / soft emphasis pulls it down
    if "gain" in dv:                                     # <prosody volume>: loud is intense, soft hushes
        g = float(dv["gain"])
        if g >= 0.9:
            inten = max(inten, min(1.0, (g - 0.9) / 0.9))
        else:
            inten *= max(0.35, g)                        # soft pulls the span down into a hush
    r = float(dv.get("speed", dv.get("rate", 1.0)))
    if r > 1.0:
        inten = max(inten, _rate_intensity(r) * 0.8)
    if dv.get("lift"):
        inten = max(inten, 0.4)
    return max(0.0, min(1.0, inten))


def _perform_kokoro(dv, pressure):
    """Resolve a <perform> span onto the PEAK envelope for the Kokoro path: peak blend
    slot + peak prosody ranges + a performance-intensity floor (wrapping a phrase performs
    it, even without an explicit <emphasis>). Returns (register, speed, gain, lift)."""
    pk = _peak_envelope()
    ppr = pk.get("prosody") or {}
    reg = max(0, min(_max_render_slot(), _env_slot(pk)))
    fi = max(_emph_intensity(dv.get("force", 1.0)), 0.85)
    force = _map_range(ppr["emphasis"], fi) if "emphasis" in ppr else 2.4
    ri = _rate_intensity(float(dv.get("speed", dv.get("rate", 1.0))))
    speed = _map_range(ppr["rate"], ri) if "rate" in ppr else float(dv.get("speed", dv.get("rate", 1.0)))
    gain = max(0.4, min(2.5, pressure * (1.0 + 0.08 * reg) * force))
    return reg, round(max(0.5, min(2.0, speed)), 3), round(gain, 3), True
...
def resolve_instructions(text, base, tone):
    ...
    for op in ops:
        if op[0] == "say":
            _, span, dv = op
            if dv.get("perform"):                       # <perform> -> render this span in peak
                reg, speed, gain, lift = _perform_kokoro(dv, tone.get("pressure", 1.0))
                out.append({"say": span.strip(), "register": reg, "gain": gain, "speed": speed,
                            "lift": lift, "perform": True, "intensity": 1.0,
            ...
            out.append({"say": span.strip(), "register": reg, "gain": round(max(0.4, min(2.5, gain)), 3),
                        ...
                        "intensity": round(_span_intensity(dv, tone.get("fill", 0.0)), 3),
                        "vrgb": directive_to_vrgb(dv, base)})
...
def parse_vox(text):
    ...
    def walk(el, ctx):
        ...
        for ch in el:
            ...
            elif tag in ("laugh", "chuckle", "giggle"):
                ops.append(("clip", tag))
            elif tag == "sub":
...
def tune_sample():
    ...
    # Expressive audition: render through Chatterbox exactly like cue-vox does, so the
    # tuner matches the LIVE voice. Play used to be Kokoro-only, so you could not hear the
    # Chatterbox exaggeration you were tuning into the peak envelope. Intensity 0..1 maps to
    # the dial 1.0..2.0 the live render expects (1.0 = range floor, 2.0 = range ceiling), so
    # intensity 1.0 auditions the boldest this envelope gets. Uses the SELECTED envelope's
    # profile (the tuner sets it active via /api/tune/envelope).
    if d.get('expressive'):
        if not _chatterbox_imported:
            return jsonify(ok=False, error='chatterbox not available (expressive path)'), 200
        envk = str(d.get('envelope', '')).strip()   # audition the SELECTED envelope, not the stale active one
        if envk:
            set_render_register(envk, 'tuner-expr', gated=False)
        try:
            inten = float(d.get('intensity', 1.0))
        except (TypeError, ValueError):
            inten = 1.0
        exdial = 1.0 + max(0.0, min(1.0, inten))
        try:
            wav = _render_expressive_to_wav(text, exdial)
        except Exception as exc:
            return jsonify(ok=False, error='chatterbox synth: %s' % exc), 200
        if not wav:
            return jsonify(ok=False, error='no audio'), 200
        ...
        def _play_expr(o):
            try:
                ...
                try:
                    ...
                    pass
        threading.Thread(target=_play_expr, args=(wav,), daemon=True).start()
        prof = _active_chatterbox_profile()
        exag = _map_range(prof["exaggeration"], max(0.0, min(1.0, exdial - 1.0))) if "exaggeration" in prof else exdial
        lab = ((_active_voice().get('envelopes') or {}).get(_active_register) or {}).get('label', _active_register)
        return jsonify(ok=True, ops=1, engine='chatterbox', exaggeration=round(exag, 2), envelope=lab)
    ...
    for op in ops:
        if op[0] == "say":
            _, span, dv = op
            if dv.get("perform"):               # <perform> auditions in the peak envelope
                regc, speed, gain, is_q = _perform_kokoro(dv, pressure)
...
- <perform>...</perform> to lift a whole phrase into the peak register for a beat or a bit, then drop back. Use it sparingly, for the line that should PERFORM.
- <laugh/> (hearty), <giggle/> (light and quick), or <chuckle/> (soft, under the breath) for a reaction.
...
def tune_deploy():
    ...
    fp = voice_fingerprint()
    _vlog('voice', 'DEPLOYED v%d -> %s (applied live) fingerprint=%s' % (PACKAGE_SCHEMA_VERSION, path, fp))
...
def tune_voice():
    ...
    # Situations are SHARED across the personality's envelopes (A/B compare: same script,
    # different envelope). They live on the personality, not per envelope.
    return jsonify(ok=True, active=_ACTIVE_VOICE, envelope=_active_register,
...
def tune_envelope():
    ...
    This is the top-level tuner selector: pick a personality, then which of its envelopes
    the engine rides. Persists the choice per voice; auditioning bypasses the expressive
    gate so any envelope can be heard on demand."""
    d = request.get_json(force=True, silent=True) or {}
    vid = (d.get('voice') or _ACTIVE_VOICE or '').strip()
    if vid != _ACTIVE_VOICE:
        set_active_voice(vid)
    key = str(d.get('envelope', '')).strip()
    envs = (_VOICE_REGISTERS or {}).get('registers', {}) or {}   # internal view = the envelopes
    if key not in envs:
        return jsonify(ok=False, error='unknown envelope: %s' % key), 200
    ...
    if v is not None:
        v['active_envelope'] = key       # remember per voice (survives boot)
        _save_voices()
    env = envs.get(key, {})
    situations = (v or {}).get('situations', []) if v else []
    return jsonify(ok=True, envelope=key, slot=_env_slot(env),
                   sample='', samples=situations)   # situations are shared across the personality


@app.route('/api/tune/situations', methods=['POST'])
def tune_situations():
    """Save the personality's SHARED situation library (A/B compare: one script, every
    register). Situations live on the personality, not per register. Body:
    {voice, situations:[{situation, ssml}]}."""
    d = request.get_json(force=True, silent=True) or {}
    vid = (d.get('voice') or _ACTIVE_VOICE or '').strip()
    sits = d.get('situations')
    v = ((_VOICES or {}).get('voices', {}) or {}).get(vid)
    if not v or not isinstance(sits, list):
        return jsonify(ok=False, error='unknown voice or bad situations: %s' % vid), 200
    v['situations'] = [{'situation': str(s.get('situation', ''))[:40], 'ssml': str(s.get('ssml', ''))}
                       for s in sits if isinstance(s, dict)]
    if not _save_voices():
        return jsonify(ok=False, error='write failed'), 200
    _vlog('voice', 'situations saved for %s (%d)' % (vid, len(v['situations'])))
    return jsonify(ok=True, voice=vid, situations=v['situations'])
...
def tune_envelopes():
    ...
    d = request.get_json(force=True, silent=True) or {}
    vid = (d.get('voice') or _ACTIVE_VOICE or '').strip()
    incoming = d.get('envelopes') or {}
    v = ((_VOICES or {}).get('voices', {}) or {}).get(vid)
    if not v or not isinstance(incoming, dict):
        return jsonify(ok=False, error='unknown voice or bad envelopes: %s' % vid), 200
    ...
    if d.get('replace'):
        ...
        # (that is how REMOVE works), keeping unsent fields on survivors via the merge.
    ...
    for key, r in incoming.items():
            ...
            for k, lo, hi in (('break', 0.25, 4.0), ('emphasis', 0.5, 3.0),
                ...
                if k in pin:
                    ...
                    if isinstance(val, (list, tuple)) and len(val) == 2:
                        ...
                        pr[k] = [s, s]
        cbx = r.get('chatterbox_profile')       # the envelope's Chatterbox performer
        if isinstance(cbx, dict):
            ...
            if isinstance(ex, (list, tuple)) and len(ex) == 2:
                a = round(_cl(ex[0], 0.3, 2.5, 0.6), 3); b = round(_cl(ex[1], 0.3, 2.5, 1.0), 3)
                cp['exaggeration'] = [min(a, b), max(a, b)]
            def _rng_field(val, lo, hi, dl, dh):
                if isinstance(val, (list, tuple)) and len(val) == 2:
                    a = round(_cl(val[0], lo, hi, dl), 3); b = round(_cl(val[1], lo, hi, dh), 3)
                    return [min(a, b), max(a, b)]
                s = round(_cl(val, lo, hi, dl), 3); return [s, s]      # scalar -> degenerate range
            if 'cfg_weight' in cbx:                 # pace: now a range (floor deliberate -> ceiling fast)
                cp['cfg_weight'] = _rng_field(cbx['cfg_weight'], 0.1, 1.0, 0.3, 0.7)
            if 'gain' in cbx:                        # loudness: a range, interpreted post-synth
    ...
    if vid == _ACTIVE_VOICE:                    # refresh the live envelope view
        _VOICE_REGISTERS = {'registers': v.get('envelopes', {})}
    _vlog('voice', 'envelopes saved for %s (%d)' % (vid, len(incoming)))
    return jsonify(ok=True, voice=vid, envelopes=v.get('envelopes', {}),
                   fingerprint=voice_fingerprint(vid))
...
def handle_audio(data):
    ...
    try:
        ...
        # Voice fingerprint query -> answer the correct live id directly (read, not
        # generated), so "what's your voice fingerprint" is always exact. Not while held.
        if not data.get('holding') and _is_fingerprint_query(text):
            try:
                os.remove(temp_file.name)
            except OSError:
                pass
        ...
        # Ladder walk -> the deterministic voice tour (no model). Gated like the benchmark.
        if not data.get('holding') and _is_ladder_command(text):
            try:
                os.remove(temp_file.name)
            except OSError:
                pass
            _confirm_slow_op(text, "ladder", {})
            return
        ...
        # ONE CEILING + SLOW TRACK: the register is not climbed by a per-turn energy pool.
        # The concurrent track manager owns which Envelope Track is active (a slow moving
        # average of token warmth, with hysteresis); apply its current decision here so the
        # gate re-asserts for the expressive state. Content fills up to it at synth time.
        if _kokoro_available:
            set_render_register(_current_track_key(), "track", apply_exag=False)
...
def handle_text_message(data):
    ...
    try:
        ...
        # Voice fingerprint query -> answer the correct live id directly (read, not
        # generated), so cue-vox always reports the exact voice it is speaking with.
        if _is_fingerprint_query(text):
            _answer_fingerprint()
            return

        # Ladder walk -> the deterministic voice tour (no model). Gated like the benchmark.
        if _is_ladder_command(text):
            _confirm_slow_op(text, "ladder", {})
            return
...
def handle_connect():
    ...
    _start_track_manager()                 # ensure the concurrent Envelope Track manager is running
    try:                                   # stamp the live voice fingerprint into the browser console on connect
        emit("voice_fingerprint", {"fingerprint": voice_fingerprint(), "voice": _ACTIVE_VOICE})
    except Exception:
        pass
    if _audit_logger:
